...
> SKK ではなかった。`set -g focus-events off` で解決済みのため、現在は
> tmux default の `C-b` で問題なく動作する。下表の「不採用」は当時の推定
> 経緯メモであり、SKK が実機で C-b 等を喰う再現はない。
...
本リポジトリは `dot_config/tmux/conf/options.conf` で `focus-events off`
にしてある。tmux-resurrect の focus 連動は犠牲になるが運用上問題なし。
