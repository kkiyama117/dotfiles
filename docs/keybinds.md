...
# = ~/.local/bin/claude-tmux-new feature/x
...
設定: `dot_config/claude/settings.json` の `hooks.*` + `~/.local/bin/claude-notify-hook` (Go binary、内部で sound + dispatcher を fork)。
...
| `C-b C, n` で popup が出ない | `~/.local/bin/claude-pick-branch` が `chezmoi apply` でビルド配置されているか / `fzf` がインストール済みか |
...
| `dot_config/tmux/scripts/` | TPM bootstrap (`executable_tpm-bootstrap.sh`) のみ。Claude/cockpit/notify 系は Go バイナリへ移行済み |
| `programs/claude-tools/cmd/` | claude 関連 Go バイナリのソース。`chezmoi apply` で `~/.local/bin/` にビルド配置 |
| `programs/claude-plugins/plugins/kkiyama117-flow-tools/commands/` | 自作 slash command (`/branch-out` `/branch-finish` `/branch-merge`) |
| `dot_config/lazygit/config.yml` | lazygit 設定（delta ページャ統合 / nvim 連携） |
| `dot_config/zsh/rc/my_plugins/tmux.zsh` | `tmux_claude_new` 関数 |
| `dot_config/claude/settings.json` | Claude Code フック登録 + プラグイン有効化 |
| `~/.local/bin/claude-notify-hook` | Claude Code hook 入口 (Go binary) |
| `~/.local/bin/claude-notify-sound` | sound 再生 worker (Go binary) |
| `~/.local/bin/claude-notify-dispatch` | popup + click action ハンドラ (Go binary、daemon fallback path) |
| `~/.local/bin/claude-notifyd` | popup state 集約常駐 daemon (Go binary, systemd socket activation) |
