...
### F-3. wired 通知の左クリック / 右クリックアクション実装 🆕 2026-04-29
...
- 設計メモ:
  ...
  - 左クリック (`default` アクション):
    ...
    - tmux 外なら fallback として `cwd` を持つ ghostty ウィンドウを `wmctrl` / `swaymsg` で focus、もしくは `transcript_path` を `$EDITOR` で開く
...
### F-4. wired-notify を chezmoi run_once で管理対象化 🆕 2026-04-29
