...
- G-1 派生フォローアップ (本 spec の Out-of-Scope, G-1 完了後に着手):
  ...
  - [x] **G-1.next #7: dispatchKill の testability refactor (L-2 残り) — 完了 (2026-05-02 / `feat/golang-convert-next`)** — `cmd/claude-cockpit-switcher/main.go` で `confirmYesNo()` を `Prompter` interface (`Confirm(prompt string) bool`) と `ttyPrompter` 実装型に分離、`dispatchKill` シグネチャに `prompter` 引数を追加して `/dev/tty` 直 open を注入境界に隔離。test 側は `recordingRunner` を `register()` メソッドで argv 別 response を返せるよう拡張、`fakePrompter` を新設して `dispatchKill` の P/W (managed/unmanaged) /S 各経路を 7 ケース (P-yes/no, W-unmanaged-yes/no, W-managed-no, S-yes/no, unknown-kind) でテーブル網羅。W-managed-yes は外部 `claude-kill-session.sh` を `exec.CommandContext` で直に蹴り出す経路で interactive smoke 領域なので unit test 対象外 (本コミットのコメントに明記)。`go test -race ./...` 全 PASS / switcher coverage 51.6% (refactor 前未計測)。
  - [x] **G-1.next #8: `eventToStatus` の YAGNI 削除 (L-5) — 完了 (2026-05-02 / `feat/golang-convert-next`)** — `cmd/claude-cockpit-state/main.go` の thin wrapper を削除、`TestEventToStatus` を `TestEventToAction` に書き換え (`actionRemove` の `SessionEnd` 経路もテーブル駆動に追加して直接検証)。`go test -race ./...` 全 PASS。
...
- 移行順序 (1 PR = 1 binary):
  ...
  - [x] PR-C-4: `claude-tmux-new` (2026-05-02) — `internal/tmux` に session/window 作成系 API 追加 (`HasSession` / `NewSessionDetached` / `NewWindowSelectExisting` / `SetWindowOption` / `SplitWindowH` / `SelectPaneTitle` / `SwitchClient` / `AttachSessionExec` (`syscall.Exec` で TTY 移譲)) / `internal/gitwt` に worktree mutation API 追加 (`HasLocalRef` / `HasRemoteRef` / `AddExistingLocal` / `AddTrackingRemote` / `AddFromHead`) / `cmd/claude-tmux-new` で 226 行 shell を Go 移植 (`parseArgs` 副作用なし testable seam / `run()` runtime entry / `resolveWorktree` の 3-path state machine: existing → local ref → remote ref → HEAD / `pickRootSession` は fzf cancel を nil error で返す / `claudeWorktreeHistoryExists` で `~/.claude/projects/<encoded>/*.jsonl` 確認 / `setupNewWindow` で 2-pane split + `claude --continue/--resume/--fork-session` 経路 + `tmux.ShellQuote` で `--prompt` を POSIX 単引用 / TMUX 内なら `SwitchClient`、外なら `AttachSessionExec` で TTY 引継ぎ)。binary は `claude-` prefix 統一のため `tmux-claude-new` から `claude-tmux-new` にリネーム。unit test 23 本 (tmux +6 / gitwt +4 / claude-tmux-new 2 表 = parseArgs 10 ケース + buildClaudeCommand 5 ケース) PASS、`go test -race ./...` 23 パッケージ全 PASS、`go vet ./...` clean、`go build ./cmd/claude-tmux-new` OK、旧 `dot_config/tmux/scripts/executable_tmux-claude-new.sh` は `git rm` で撤去 (`bindings.conf` は `tmux-claude-new` を直接呼ばず pick-branch 経由 → C-5 で更新)。実機 tmux 目視 smoke は user 手動にデファ ([`smoke/2026-05-02-go-tmux-scripts-smoke.md`](superpowers/smoke/2026-05-02-go-tmux-scripts-smoke.md))
    - [x] (follow-up at C completion) `docs/manage_claude.md` / `docs/keybinds.md` / `docs/shell_discovery.md` の `tmux-claude-new.sh` 言及を `claude-tmux-new` に更新 (2026-05-03 に `manage_claude.md` §5.2/5.3/5.7 + `keybinds.md` §I-2.5/I-4/I-5/I-6 + §2.2 を一括更新。`shell_discovery.md` は `tmux_claude_new` zsh ラッパ概念のみ言及で実体名なしのため変更不要)
    - [ ] (follow-up at C completion) `cmd/claude-tmux-new` のカバレッジが 24.5%。`run()` / `resolveWorktree` / `claudeWorktreeHistoryExists` / `setupNewWindow` の単体テストを追加して 80% 規約に到達させる
    - [ ] (follow-up at C completion) `pickRootSession` の `bash -c "ls -t %q/*.jsonl | fzf ..."` 部分を pure-Go (`filepath.Glob` + `exec.Command("fzf", ...)`) に書き直し、`%q`/shell-injection 懸念を排除
    - [ ] (follow-up at C completion) `internal/gitwt` (74.6%) / `internal/tmux` (72.1%) のカバレッジを 80% に到達させる。残差は `gitwt.Remove` (`exec.CommandContext` 直叩きで stderr 捕捉、design trade-off) と `tmux.AttachSessionExec` (`syscall.Exec`) — interface 抽象化で test 可能化を検討
  - [x] PR-C-5: `claude-pick-branch` (2026-05-02) — `internal/gitwt` に `LocalBranches` 追加 (`git for-each-ref --format=%(refname:short) refs/heads` を proc.Runner 経由で叩き、empty 時は nil slice を返す / `fmt.Errorf("git for-each-ref: %w", err)` で wrap) + table test 2 本 (3 branch list / empty) / `cmd/claude-pick-branch/main.go` 実装: `exec.LookPath("fzf")` で fzf 必須 (無ければ `tmux display-message` + stderr + exit 1) → `os.Getwd()` + `gw.LocalBranches` でローカル branch 列挙 (空なら display-message + exit 0) → `os.Args[1:]` を passthrough として保持 → `promptForFlags(passthrough)` で `--no-claude` 含有時のみ `worktree branch> ` を選択 (default は `claude branch> `) → `runFzf(stdin, prompt)` で fzf を `cmd.Output()` で起動 (cancel または empty pick は silent exit 0) → `exec.LookPath("claude-tmux-new")` で binary 解決 → `syscall.Exec(bin, buildExecArgs(pick, passthrough), os.Environ())` で TTY を移譲 (argv[0] に "claude-tmux-new" を prepend する syscall convention 厳守)。table test 2 本 (buildExecArgs 3 ケース: branch only / --no-claude / multiple flags + promptForFlags 2 ケース: default / --no-claude switch) PASS、`go test -race ./...` 24 パッケージ全 PASS (新規 cmd/claude-pick-branch 含む)、`go vet ./...` clean、`go build ./cmd/claude-pick-branch` OK、`dot_config/tmux/conf/bindings.conf` L61 (`prefix + C → n`) と L66 (`prefix + C → o, --no-claude`) を `~/.local/bin/claude-pick-branch` に書き換え、旧 `dot_config/tmux/scripts/executable_claude-pick-branch.sh` は `git rm` で撤去。実機 tmux 目視 smoke は user 手動にデファ ([`smoke/2026-05-02-go-tmux-scripts-smoke.md`](superpowers/smoke/2026-05-02-go-tmux-scripts-smoke.md))
  - [x] **Phase C 完走** (2026-05-02) — `tmux scripts → Go` 移行完了。`go test -race -cover ./...` で 24 パッケージ全 PASS。`internal/{cockpit,notify,notifyd,obslog,proc,xdg}` は 80%+ coverage 達成。旧 shell 5 本 (`claude-{branch,respawn-pane,kill-session,pick-branch}.sh`, `tmux-claude-new.sh`) は `git ls-files` から完全撤去確認、tmux conf 内 `.config/tmux/scripts/.*\.sh` 参照は `tpm-bootstrap.sh` (D subsystem 繰延) のみ。spec acceptance §11 のうち automated 層 (test/build/lint/grep) は全部 GREEN、実機 tmux 目視 smoke (5 領域 + 通し) のみ user 手動にデファ ([`smoke/2026-05-02-go-tmux-scripts-smoke.md`](superpowers/smoke/2026-05-02-go-tmux-scripts-smoke.md))

#### G-2.next (D subsystem 候補 — Phase 4 着手前に判断)

- [ ] **D-1**: `tpm-bootstrap.sh` の Go 化検討。network I/O (git clone TPM) と `~/.config/tmux/plugins/tpm/` 配置のみで、`.chezmoiscripts/run_once_all_os.sh.cmd.tmpl` から 1 回呼ばれるだけ。Go 化メリットは小さい (大半が git clone 副作用) ため YAGNI でシェルのまま放置も選択肢。判断ポイントは Phase 4 着手時に「他の `run_once_*.sh.tmpl` 群と一括 Go 化するか」
- [ ] **D-2**: `.chezmoiscripts/run_*.sh.tmpl` 全体の Go 化方針検討。Bitwarden hook (`.executable_password_manager.sh`) や bootstrap 系を含む。chezmoi template との結合度が高く、design 別出しが必要 (本 spec の対象外)
- [ ] **G-3** (low): `claude-tools/` 配下 binary の log file を `/tmp/*.log` から `~/.cache/claude-tools/<bin>.log` に移動。`xdg.CacheDir()` ベース、複数ユーザー / WSL 対応の見栄え改善
