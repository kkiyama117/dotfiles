...
- **G3 階層スイッチャ**: `prefix + S` で session → window → pane を 1 popup で扱える fzf スイッチャを提供
- **G4 Next-ready ジャンプ**: `prefix + N` で `done` 状態の pane に inbox 順で循環移動
...
├── executable_switcher.sh                       prefix+S が呼ぶ階層 fzf
├── executable_next-ready.sh                     prefix+N が呼ぶ
...
dot_config/tmux/conf/bindings.conf              ← prefix+S, prefix+N 追加 / claude_table.s 差替え
...
- prefix+S         → cockpit/switcher.sh        (fzf popup)
- prefix+N         → cockpit/next-ready.sh      (one-shot jump)

orphan cleanup:
  tmux server 起動時 / prefix+S 起動冒頭 / Ctrl-R 明示再描画 → cockpit/prune.sh
```
...
| `cockpit/next-ready.sh` | done 状態の pane に循環移動 | tmux server, `panes/*.status` | tmux switch-client + select-pane |
...
### 6.4 Next-ready（`prefix + N`）

巡回範囲: **全 tmux session 横断**（claude-* に限定しない。状態ファイルが `done` のものすべてが対象）。

inbox order = `session 名昇順 → window-index 昇順 → pane-index 昇順`。done 状態の pane を列挙し、現在 pane の **inbox 順での次の done pane**（リスト末尾なら先頭に循環）にジャンプ。done が 0 件なら `tmux display-message "no ready claude pane"` で 1 秒間メッセージ表示。
...
| `prefix + S` | （未定義） | 階層スイッチャ起動（**新設、上流互換**） |
| `prefix + N` | （未定義） | next-ready ジャンプ（**新設**） |
| `prefix + C s` (claude_table.s) | claude-pick-session.sh = claude-* フラット fzf | **`switcher.sh` を呼ぶ**（バインド名は維持） |
| 他 claude_table キー (c/n/r/k) | 既存挙動 | **不変** |
...
| Claude が SIGKILL → Stop hook 不発 | 状態が `working` のまま残る。`prune.sh` 後の次の `prefix+S` または kill-session 時に消える。**許容**（誤検知より誤陰性のほうが安全） |
...
5. `prefix + N` で done 側 pane にジャンプできる
6. `prefix + S` で階層スイッチャを開き、`Ctrl-X` で空 pane を kill できる
...
4. `dot_config/tmux/conf/bindings.conf` に `prefix+S` / `prefix+N` を追加、`claude_table.s` を差替え
