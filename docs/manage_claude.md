...
ターン完了 / 入力待ちで **音 + デスクトップ popup** を出す。hook 入口は `claude-notify-hook` (Go binary) で、内部で sound 再生と popup dispatch を行う。G-1.next #3 (2026-05-02) で常駐 daemon `claude-notifyd` を導入し、pipeline は以下の形になった:
...
Claude Code ─┐ (event)
  ...
  claude-notify-hook (Go, exit 0 fast)
             │
             ├── fork: claude-notify-sound (音再生、数秒で終了)
             │
             ├── [primary] Unix socket → ${XDG_RUNTIME_DIR}/claude-notify/sock
             │              ↓ systemd socket activation
             │   claude-notifyd (常駐 daemon, 1 process per UID)
             │              ↓ godbus
             │   D-Bus session bus → wired-notify popup
             │              ↓ ActionInvoked signal
             │   tmux switch-client + select-pane / WM focus (xdotool/swaymsg)
             │
             └── [fallback] setsid fork: claude-notify-dispatch
                   (daemon 停止中 / socket dial 100ms timeout 時のみ)
```

`claude-notifyd` は `claude-notifyd.socket` (systemd socket activation) で常駐し、hook からの 1 行 JSON フレームを受け取って D-Bus 経由で wired-notify に popup を送る。daemon が停止中または socket dial が 100ms 以内に失敗した場合は従来の `claude-notify-dispatch` fork にフォールバックし、popup 到達は保証される。

詳細設計: `docs/superpowers/specs/2026-05-02-notify-dispatch-daemon-design.md`。

内部で sound 再生 (`claude-notify-sound`) と dispatch (socket または fallback fork) を行う構成:
